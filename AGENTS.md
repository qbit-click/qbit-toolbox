...
- Use `export`/`import` only for deliberate offline or cross-machine continuity transfer. After an imported offline session advances context locally, use `reconnect` when the configured remote is reachable; never bypass a reconnect divergence by reset, rebase, merge, or force-push.
